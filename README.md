...
This bounded models checker considers only infinite traces that can be represented in the lasso form: a finite prefix followed by an infinitely repeated lasso. This is the same semantics that complete (non-bounded) model checkers assume for normal LTL.
...
The checker translates the model and the HyperLTL property into a set of constraints using quantifiers that are solved using the Z3 SMT solver. Unlike other existing bounded model checkers for HyperLTL (namely [HyperRUSTY](https://github.com/HyperQB/HyperRUSTY/)), our tool fully supports loop conditions so the results are complete for the given trace length.
